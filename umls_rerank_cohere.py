...
umls_api = UMLS_API("dc0eff65-4588-4030-b28d-a70866df0a97")
umlsbert = UMLSBERT()
cohere_reranker = UMLS_CohereReranker("EGG2adUrar0cWM8wdQwmfiMf8mJSm2jSzqDj0QUy")

def get_umls_keys(query, prompt, llm):
    umls_res = {}
    prompt = prompt.replace("{question}", query)
    print("PROMPT:", prompt)
    #eys_text = llm.invoke(prompt)
    #
    # 
    # print(keys_text)
    try:
        #keys_text = llm.predict(prompt)
        keys_text = llm.invoke(prompt)
        print("keys_text:", keys_text)
        #pattern = r"\{(.*?)\}"
        pattern = r"\*\*({.*?})\*\*"  
        ...
        json_data = matches[0]
        json_data = json_data.replace("'", "\"").strip()  # Reemplazar comillas simples con dobles
        print("Cleaned JSON:", json_data)
        
        #keys_dict = json.loads("{" + matches[0] + "}")
        keys_dict = json.loads(json_data)
        print("Keys dict:", keys_dict)
    ...
    for key in keys_dict["medical terminologies"][:]:
        ...
        if relations is not None:
            ...
            if top_rel_texts:
                reranked_rel_texts = cohere_reranker.rerank(query, top_rel_texts) #APIIIII
            else:
                reranked_rel_texts = [] 
